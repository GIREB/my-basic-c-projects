...
void deleting_ParticularPosition(int x){
    if (start->data==x)
    {
        deletionatbeginning();
    }
...
void Reverse_iterativemethod(){
    ...
    while (q!=NULL)
    {
        temp1=q->next;//store the next node in temp
        q->next=previous;//revrese the next node of q in temp
        previous=q;//move the previous to q
        q=temp1;//move the q to temp
    }
    start=previous;
}
void Print_Nodes(){
...
int main(){
    ...
    while (1)
    {
        ...
        printf("6)delete from particular position where you want\n");
        printf("7)reverse the nodes...\n");
        ...
        case 6:
            ...
            scanf("%d",&selection);
            deleting_ParticularPosition(selection);
            Print_Nodes();
            break;
        case 7:
            Reverse_iterativemethod();
            Print_Nodes();
            break;
        }
    }
